...
async def search_hybrid(pool, query, embedding, top_k=8):
    ...
    async with pool.acquire() as conn:
        try:
            rows = await conn.fetch("SELECT * FROM hybrid_search($1, $2::vector, $3)", query, emb_str, top_k)
...
async def evaluate(use_hybrid=False, top_k=8, verbose=True):
    ...
    for item in golden:
        ...
        emb = await embed(q)
        rows = await (search_hybrid(pool, q, emb, top_k) if use_hybrid else search_vector(pool, q, emb, top_k))
        ...
        if verbose:
            ...
            print(f"[{status}] {item['id']:18} rank={rank_str:4} mrr={mrr:.3f} q={q[:50]:50} top={results[-1]['top_files'][0] if results[-1]['top_files'] else '—'}")
            if not passed:
                print(f"       expected file_contains={exp.get('file_contains')} chunk_contains={exp.get('chunk_contains')} project={exp.get('project_id','*')}")
                for i, r in enumerate(rows[:3], 1):
                    print(f"       {i}. {r['project_id']}/{r['file_path']} :: {r.get('chunk_name') or ''} sim={r.get('similarity', r.get('rank',0)):.3f}")

    chunk_count = {r["project_id"]: r["n"] for r in await pool.fetch(
    ...
    if verbose:
        ...
        print(f"Passed: {summary['passed']}/{n}  Recall@1 {summary['recall@1']:.2%}  Recall@3 {summary['recall@3']:.2%}  Recall@8 {summary['recall@8']:.2%}  MRR {summary['mrr']:.3f}")
        print("="*70)
        if summary["passed"] < n:
...
def main():
    ...
    args = ap.parse_args()
    summary, results = asyncio.run(evaluate(use_hybrid=args.hybrid, top_k=args.top_k, verbose=not args.json))
    if args.json:
