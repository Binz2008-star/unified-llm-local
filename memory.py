...
class MemoryManager:
    def __init__(
        self, pool: asyncpg.Pool = None, memory_dir: str = "./memory", on_reload: Callable = None
    ):
        self.pool = pool
    ...
    def _start_watcher(self):
        ...
        if _shared_observer is None:
            ...
            print("[Memory] Using shared file watcher")
            ...
            try:
                async with self.pool.acquire() as conn:
                    existing = await conn.fetchval(
                        "SELECT id FROM memory WHERE content_hash = $1 LIMIT 1", fp
                    )
            ...
            async with self.pool.acquire() as conn:
                try:
                    ...
                    if row is None:
                        ...
                        existing = await conn.fetchval(
                            "SELECT id FROM memory WHERE content_hash=$1", fp
                        )
                        return existing
                    return row["id"]
                except Exception:
    ...
    def auto_learn_from_conversation(
        self, user_msg: str, assistant_msg: str, tool_results: list[str]
    ):
