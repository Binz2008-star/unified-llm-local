    ...
    if name == "search_brain":
        ...
        if not task:
            return types.CallToolResult(
                isError=True, content=_text("Missing required argument: task")
            )
        await ba.run_multi_agent(task)
        return types.CallToolResult(
            content=_text(json.dumps({"status": "completed", "task": task}))
        )
        ...
        if not target_file or not patch_content:
            return types.CallToolResult(
                isError=True,
                content=_text("Missing required arguments: target_file and patch_content"),
            )
        ...
        if not target_file or not search_block or not replace_block:
            return types.CallToolResult(
                isError=True,
                content=_text(
                    "Missing required arguments: target_file, search_block, replace_block"
                ),
            )
...
server = Server("second-brain-v4", on_list_tools=_handle_list_tools, on_call_tool=_handle_call_tool)
