...
def should_index(file_path: Path, root: Path, extra_ignore=frozenset()):
    ...
    if (
        lname in ("token.json", "client_secret.json", ".env")
        or lname.startswith("client_secret")
        or "credential" in lname
        or lname.endswith(".pem")
...
def resolve_edges(project_root: Path, rel_file: str, text: str):
    ...
    {p.as_posix() for p in (project_root / "src").rglob("*")} if src_root.exists() else set()
...
def collect_files(project_id):
    ...
    files = [f for f in walk_pruned(root) if f.is_file() and should_index(f, root, extra)]
    return root, files
